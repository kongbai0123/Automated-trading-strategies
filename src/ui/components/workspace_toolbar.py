...
def render_workspace_toolbar(
    *,
    ...
    col_symbol, col_timeframe, col_period, col_strategy, col_mode = st.columns([1.5, 1, 1, 1.4, 1.1])
    with col_symbol:
        symbol = st.selectbox("Symbol", list(symbols), index=list(symbols).index(selected_symbol))
    with col_timeframe:
        timeframe = st.selectbox("Timeframe", list(timeframes), index=list(timeframes).index(selected_timeframe))
    with col_period:
        period = st.selectbox("Period", list(periods), index=list(periods).index(selected_period))
    with col_strategy:
        strategy_name = st.selectbox("Strategy", list(strategies), index=list(strategies).index(selected_strategy))
    with col_mode:
