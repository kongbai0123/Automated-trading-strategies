...
class BollingerBreakoutStrategy(Strategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        if self.close_col not in df.columns or self.upper_col not in df.columns or self.lower_col not in df.columns:
            raise ValueError(f"Required columns missing for Bollinger Strategy.")
