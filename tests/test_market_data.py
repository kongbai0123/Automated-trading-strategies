...
def test_yfinance_sqlite_error_retries_without_cache(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_live_fetch_failure_falls_back_to_local_csv(tmp_path: Path) -> None:
    ...
    service = MarketDataService(
        live_provider=YahooFinanceProvider(download_fn=lambda *_args, **_kwargs: (_ for _ in ()).throw(OperationalError("disk I/O error"))),
...
def test_market_data_service_raises_controlled_error_when_all_sources_fail(tmp_path: Path) -> None:
    service = MarketDataService(
        live_provider=YahooFinanceProvider(download_fn=lambda *_args, **_kwargs: (_ for _ in ()).throw(OperationalError("unable to open database file"))),
        local_provider=LocalCsvProvider(data_dir=tmp_path / "data"),
    )
